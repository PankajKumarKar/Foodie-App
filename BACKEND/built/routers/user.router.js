        ...
        switch (_c.label) {
            ...
            case 3:
                if (_b) {
                    ...
                    res.status(http_status_1.HTTP_BAD_REQUEST).send("Email or Password is not valid !");
                }
                return [2 /*return*/];
        }
    });
...
var generateResponseToken = function (user) {
    var token = jsonwebtoken_1.default.sign({ id: user.id, email: user.email, isAdmin: user.isAdmin }, process.env.JWT_SECRET_CODE, {
        expiresIn: "30d",
    });
    return {
        id: user.id,
